...
#         grp =CreateStockGrp.objects.get(id=pk,comp = t_id)

#         item = stock_itemcreation.objects.filter(under_id = grp.id,company = t_id).values()
...
#         if item.exists():
#             if stock_item_voucher.objects.filter(company = t_id).exists():
